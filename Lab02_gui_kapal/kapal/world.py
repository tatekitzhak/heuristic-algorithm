...
class World2d(World):
    ...
    def __init__(self, costs=None, state_type=State2d, diags=True, diags_mult=1.42):
        ...
        for i in range(-1, 2):
            ...
            for j in range(-1, 2):
                ...
                if not self.in_bounds(y, x):    # out of bounds
                    continue
                if y == s.y and x == s.x:   # self cann't have self as neighbor
                    continue
                cost = self.costs[y][x]
    ...
    def in_bounds(self, y, x):
        ...
        if (y >= 0 and y < size_y and x >= 0 and x < size_x):
            return True
        else:
            return False
